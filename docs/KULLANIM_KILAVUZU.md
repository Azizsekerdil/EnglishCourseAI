...
Sürüm 1.2.0 · Windows ve macOS masaüstü uygulaması · Arayüz dilleri: Türkçe ve English
...
Bu kılavuz **English Course AI** sürüm **1.2.0** içindir. Uygulama, ana dili Türkçe olan öğrenciler için hazırlanmış, verilerini sizin bilgisayarınızda tutan bağımsız bir İngilizce çalışma ortamıdır. Tekrar, sözlük, laboratuvarlar, sınav, PDF notları ve ilerleme internet olmadan çalışır; yapay zeka özellikleri isteğe bağlıdır.
...
Pencere 1360 × 860 boyutunda açılır (en küçük 1080 × 700) ve başlıkta `English Course AI 1.2.0` yazar. Solda gruplanmış sayfa listesi, üstte araç şeridi, altta durum çubuğu vardır. Üst şeritte sayfa başlığı, `🌐` ile **arayüz dili** kutusu (`Türkçe` / `English`), yeni profil açan **+** düğmesi, **profil** kutusu ve sağ uçta `AI: Kullanılabilir` / `AI: Kullanılamıyor` rozeti bulunur.
...
| **v1.2.0** | **Yön seçici**: `Otomatik`, `EN → TR`, `TR → EN`; sabit yönde yalnızca kaynak taraf aranır ve seçim ayarlara kaydedilir. Türkçe sözlük motorunda tam bir üçüncü dil olarak ele alınır; CSV içe aktarımı başlık satırını tanır ve sütunları herhangi bir sırada kabul eder; Türkçe sorgular ASCII klavyeyle ya da büyük harfle yazıldığında da bulunur (`IŞIK` = `ışık` = `isik`) |
