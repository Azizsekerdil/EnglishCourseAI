...
class Entry:
    ...
    @property
    def plural(self) -> str:
        """Plural form kept in ``extra`` after the article/gender code; an IPA transcription (``/haʊs/``, EN app) is never a plural."""
        toks = self.extra.split()
        if self.pos != "n":
            return ""
        toks = [t for t in toks if t not in ARTICLES and t not in GENDERS and not t.startswith(("/", "["))]
        return " ".join(toks)
...
# Side credited with a match found only in ``Entry.note``: the note is a plain-English definition in the English app
# (so it belongs to the English headword side) and an English usage note in the DE/FR apps (their translation side).
NOTE_SIDE = "headword" if C.TARGET_LANG == "en" else "translation"
...
def first_sense(text: str) -> str:
    """The first sense of a ``;``/``/``-separated list as written (``"ev (bina); hane"`` -> ``"ev (bina)"``), ``""`` when empty.

    Split exactly like :func:`_senses`, so searching for the result on the side it came from is an exact hit."""
...
class Dictionary:
    ...
    A duplicate that carries a Turkish gloss the stored entry lacks fills that gloss in (the stored entry keeps
    its own source): this is how a cached AI row adds Turkish to a built-in entry across restarts."""
    ...
    def extend(self, entries: Iterable[Entry]) -> int:
        ...
        for e in entries:
            ...
            if at is not None:
                if HAS_TR and e.tr and not self._entries[at].tr:
    ...
    def would_change(self, entry: Entry) -> bool:
        ...
        return twin is None or bool(HAS_TR and entry.tr and not twin.tr)
    ...
    def lookup(self, query: str, direction: str = "auto", limit: int = 200) -> tuple[str, list[Entry]]:
        ...
        ``direction`` is a code from :data:`DIRECTIONS`. A fixed code searches only its source side (headword and
        definition for ``en2tr``, Turkish senses for ``tr2en``) and is returned unchanged. ``auto``
    ...
    def random_entry(self, rng: random.Random | None = None, field: str | None = None) -> Entry | None:
        """A random built-in entry (any entry when there is none). With ``field`` (``headword`` | ``translation`` | ``tr``)
        only entries whose side ``field`` is known are drawn, so that a fixed direction can search for the pick on its
        source side; when no entry has that side yet the pool falls back to every entry."""
        pool = [e for e in self._entries if e.source == SOURCE_BUILTIN] or list(self._entries)
        if field:
            pool = [e for e in pool if getattr(e, field, "")] or pool
...
_CONVENTIONS = {
    ...
    "en": ('"extra": the IPA transcription in slashes like "/haʊs/" or "". The translation must be Turkish. '
           '"note" must be a short plain-English definition; irregular forms are appended to it after " — ", '
           'like "to move to another place — went, gone" or "a young person — plural: children".'),
...
def ai_prompt(query: str, ui_lang: str = "en") -> str:
    ...
    # The per-key line must agree with the convention below: the EN app wants a plain-English definition, not a Turkish note.
    note_desc = ('a short plain-English definition (irregular forms appended after " — ")' if C.TARGET_LANG == "en"
                 else f"a short usage note ({other})")
    if HAS_TR:
...
def parse_ai_entries(text: str) -> list[Entry]:
    ...
    for item in data:
        ...
        # Which key feeds ``Entry.translation`` depends on the app: it is the English side for DE/FR ("translation_en",
        # legacy "translation") and the Turkish side for the English app ("translation_tr", legacy "translation").
        # The English app never reads "translation_en" - a model that adds the trilingual keys would otherwise put an
        # English gloss into the Turkish column and the real Turkish gloss would be lost (and cached that way).
        if HAS_TR:
            translation = _dedupe_senses(_clean(item.get("translation_en") or item.get("translation"), _AI_LIMITS["translation"]))
            tr = _dedupe_senses(_clean(item.get("translation_tr"), _AI_LIMITS["tr"]))
        else:
            translation = _dedupe_senses(_clean(item.get("translation_tr") or item.get("translation"), _AI_LIMITS["translation"]))
            tr = ""
        if not head or not translation:
            continue
