...
class AITab(BaseTab):
    ...
    def send(self):
        ...
        if task == "vision": prompt = prompt or "Read the English text in this image, transcribe it, translate it into Turkish, and explain important spelling, pronunciation or grammar."
...
class SpeakingTab(BaseTab):
    ...
    def build(self):
        self.heading(); main = self.card(); main.pack(fill="both", expand=True)
        bar = ttk.Frame(main, style="Card.TFrame"); bar.pack(fill="x")
        self.scenario = tk.StringVar(value="At a café")
        ttk.Label(bar, text=self.t("speak.scenario"), style="CardMuted.TLabel").pack(side="left")
        ttk.Combobox(bar, textvariable=self.scenario, state="readonly", values=("At a café", "At the station", "At a hotel", "In a shop", "At the doctor's"), width=24).pack(side="left", padx=8)
    ...
    def begin(self):
        ...
        prompt = f"Start a short A1 English role-play in the scenario '{self.scenario.get()}'. Speak as the other person. Ask only one short question."
...
class WritingTab(BaseTab):
    ...
    def correct(self):
        prompt = "Correct this English learner text. Show a corrected version, name the grammar rule, and give a brief explanation including spelling, word choice and punctuation when relevant:\n\n" + self.input.get("1.0", "end-1c")
