...
class SettingsTab(BaseTab):
    ...
    def build(self):
        ...
        s = self.app.settings
        self.theme = tk.StringVar(value=s["theme"]); self.goal = tk.IntVar(value=s["daily_goal"])
        self.tts = tk.BooleanVar(value=s["tts_enabled"]); self.ai = tk.BooleanVar(value=s["ai_enabled"])
        self.base = tk.StringVar(value=s["ai_base"]); self.model = tk.StringVar(value=s["ai_model"])
        self._policy_labels = {code: self.t(k) for code, k in self.POLICY_KEYS.items()}
        self.dict_ai = tk.StringVar(value=self._policy_labels.get(s.get("dict_ai", "auto"), self._policy_labels["auto"]))
        ...
        self.alt_enabled = tk.BooleanVar(value=bool(s.get("alt_enabled"))); self.alt_base = tk.StringVar(value=s.get("alt_base") or C.NIM_BASE)
        self.alt_model = tk.StringVar(value=s.get("alt_model") or C.ALT_MODEL_DEFAULT); self.alt_key = tk.StringVar(value="")
    ...
    def on_show(self):
        """Called by the shell whenever the page is selected: pick up settings changed elsewhere (dictionary toolbar, AI page)."""
        s = self.app.settings
        ...
        self.dict_ai.set(self._policy_labels.get(s.get("dict_ai", "auto"), self._policy_labels["auto"]))
        self.dict_autosave.set(bool(s.get("dict_ai_autosave", True)))
    ...
    def test_alt(self):
        ...
        def done(models):
            ...
            key_hint = "" if client.usable() else f" · {self.t('settings.alt_key_needed')}"     # the dictionary applies the same rule
            self.alt_result.configure(text=f"{self.t('settings.alt_ok')} · {len(models)} {self.t('settings.alt_models')}{found}{key_hint}")
