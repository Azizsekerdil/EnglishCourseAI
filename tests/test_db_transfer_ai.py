...
def test_search_uses_case_and_turkish_meanings(repos):
    assert any(w["target"] == "school" for w in repos.words.search("SCHOOL"))
    assert any(w["target"] == "child" for w in repos.words.search("çocuk"))
    assert any(w["target"] == "milk" for w in repos.words.search("süt"))
...
def test_csv_roundtrip_preserves_unicode(repos, tmp_path):
    path = tmp_path / "english-türkçe.csv"; export_csv(repos.words.search("school"), path)
    text = path.read_text(encoding="utf-8-sig"); assert "school" in text and "okul" in text
    other = Repos(Database(tmp_path / "import.db")); before = other.words.count(); import_csv(other.words, path)
    assert any(w["target"] == "school" for w in other.words.search("school")); other.db.close()
