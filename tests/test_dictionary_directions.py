...
from eca.db import Database, Repos
...
def test_parse_ai_entries_accepts_translation_en_tr_and_legacy_translation():
    rows = D.parse_ai_entries(json.dumps([
        ...
        {"headword": "Eee", "pos": "n", "extra": "", "translation": "ev", "translation_en": "house"},
        {"headword": "Fff", "pos": "n", "extra": "", "translation_en": "only english"},
    ]))
    ...
    if D.HAS_TR:
        ...
        assert by["Ccc"].translation == "new" and by["Ccc"].tr == "ağaç"                 # translation_en beats the legacy key; "ağac" is a diacritic duplicate
        assert by["Eee"].translation == "house" and by["Eee"].tr == "" and by["Fff"].translation == "only english"
        assert "Ddd" not in by                                                          # English is required
        long = D.parse_ai_entries(json.dumps([{"headword": "x", "translation_en": "y", "translation_tr": "z" * 500}]))[0]
        assert len(long.tr) <= 200
    else:
        # The English app's translation column IS Turkish: the Turkish-bearing keys win and "translation_en" is never
        # read - an English gloss must not land in the Turkish column (it would be cached and served offline that way).
        assert by["Aaa"].translation == "ev; hane" and by["Aaa"].tr == ""
        assert by["Ccc"].translation == "ağaç"                                          # translation_tr beats the legacy key, translation_en is ignored ("ağac" is a diacritic duplicate)
        assert by["Ddd"].translation == "only turkish"                                  # the English app's translation is Turkish anyway
        assert by["Eee"].translation == "ev"                                            # the legacy (Turkish) key is kept next to translation_en
        assert "Fff" not in by and all(e.tr == "" for e in rows)                        # an English-only answer is no Turkish gloss
        assert all("house" not in e.translation and "english" not in e.translation for e in rows)


def test_random_entry_can_be_drawn_by_a_known_side(dic):
    """A fixed direction searches only its source side, so the random pick must have that side (``tr`` may be unknown)."""
    rng = random.Random(3)
    assert D.first_sense("ev (bina); hane") == "ev (bina)" and D.first_sense("de/da; fazla") == "de" and D.first_sense(" ; x") == "x"
    assert D.first_sense("") == "" and D.first_sense(None) == ""
    assert all(dic.random_entry(rng, "translation").translation for _ in range(10)) and dic.random_entry(rng) in dic.entries
    for field in ("headword", "translation", "tr") if D.HAS_TR else ("headword", "translation"):
        picks = {dic.random_entry(rng, field) for _ in range(40)}
        assert picks and all(getattr(e, field) for e in picks)
        for e in picks:                                                                 # the first sense of that side finds the pick on that side alone
            q = e.headword if field == "headword" else D.first_sense(getattr(e, field))
            assert e in dic.lookup(q, D.direction_code(D._LANG_OF_FIELD[field], T if field != "headword" else D.OTHER_LANG))[1]
    if D.HAS_TR:
        assert {e.headword for e in picks} == {HOUSE, TREE, GO}                         # CHAIR has no Turkish gloss yet
        assert D.Dictionary([D.Entry(CHAIR, "n", "", "chair")]).random_entry(rng, "tr").headword == CHAIR   # nothing has the side: any entry
    assert D.Dictionary().random_entry(rng, "translation") is None
