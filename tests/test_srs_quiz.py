...
from eca.quiz_engine import Question, accuracy, build_session, check_answer
from eca.srs import SRSState, is_due, leitner_due, mastery, review
...
def test_quiz_strict_spelling_and_scoring():
    q = Question("type_in", "olumsuz", "don't", [])
    assert check_answer(q, "don't") and not check_answer(q, "dont")
    assert accuracy([True, False, True, True]) == 75.0


def test_session_generation():
    words = [{"target": "house", "tr": "ev"}, {"target": "book", "tr": "kitap"},
             {"target": "door", "tr": "kapı"}, {"target": "train", "tr": "tren"}]
